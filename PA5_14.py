...
items = [map(float, x.split(' ')) for x in open('PA5_17.txt', 'r').read().split('\n')[0:-1]]
...
@autojit
def calc(sets1, sets2, A1, A2,dist, index1, index2):
    #index2=sets2.index
    #index1=sets1.index
    for setx in sets2:
        i2=index2(setx)
        for j in setx:
            ...
            #print time.time()-st
            A2[j,i2]=round(min(np.add(A1[y,i1],dist[y,j])),2)
    return A2
...
def tsp3mat(items):
    ...
    for m in range(1, N):
        ...
        if m<2:
            ...
            A2=np.zeros([N+1, len(sets2)])+float('+inf')
            print sets2[0], 'sets2[0]', len(sets2)
            #A2=calc(sets1, sets2, A1, A2,dist)
            
            index2=sets2.index
            index1=sets1.index
            
            A2=pool.map(calc, (sets1, sets2, A1, A2,dist, index1, index2))
            #for setx in sets2:
             #   i2=index2(setx)
              #  for j in setx:
               #     if j!=1:
